...
class DummyRFNet(nn.Module):
    """Lightweight mock network imitating RFNet Model signature for fast unit tests."""
    ...
    def forward(self, x, mask):
        # x is in RFNet order [FLAIR, T1ce, T1, T2]
        # mask is (B, 4) boolean
...
def test_rfnet_adapter_init():
    dummy = DummyRFNet(num_cls=4)
    adapter = RFNetAdapter(device="cpu", patch_size=(32, 32, 32), network=dummy)
...
def test_rfnet_adapter_from_config():
    cfg = Config({
        "paths": {"rfnet_weights": "checkpoints/rfnet_best.pth"},
        "device": "cpu",
        "patch": {"size": [32, 32, 32]},
        "model": {"num_classes": 4},
    })
    adapter = RFNetAdapter.from_config(cfg)
    assert adapter.patch_size == (32, 32, 32)
    assert adapter.num_classes == 4


def test_rfnet_adapter_predict_4d_and_5d():
    dummy = DummyRFNet(num_cls=4)
    adapter = RFNetAdapter(device="cpu", patch_size=(32, 32, 32), network=dummy)
...
def test_rfnet_adapter_predict_logits():
    dummy = DummyRFNet(num_cls=4)
    adapter = RFNetAdapter(device="cpu", patch_size=(32, 32, 32), network=dummy)
...
def test_rfnet_adapter_scenarios_mask():
    dummy = DummyRFNet(num_cls=4)
    adapter = RFNetAdapter(device="cpu", patch_size=(32, 32, 32), network=dummy)
    ...
    # in RFNet order [FLAIR, T1ce, T1, T2], FLAIR is index 0 -> False
...
def test_rfnet_adapter_auto_missing_mask():
    dummy = DummyRFNet(num_cls=4)
    adapter = RFNetAdapter(device="cpu", patch_size=(32, 32, 32), network=dummy)
    ...
    # In RFNet order [FLAIR, T1ce, T1, T2], T1ce is index 1
...
def test_rfnet_adapter_dict_input():
    dummy = DummyRFNet(num_cls=4)
    adapter = RFNetAdapter(device="cpu", patch_size=(32, 32, 32), network=dummy)
    sample_dict = {
...
def test_rfnet_adapter_raw_3channel_tensor_with_scenario():
    """Verify raw 3-channel tensor is correctly expanded to 4 channels with mask='S1'."""
    dummy = DummyRFNet(num_cls=4)
    adapter = RFNetAdapter(device="cpu", patch_size=(16, 16, 16), network=dummy)
...
def test_rfnet_adapter_evaluate_sample():
    dummy = DummyRFNet(num_cls=4)
    adapter = RFNetAdapter(device="cpu", patch_size=(32, 32, 32), network=dummy)
    x = torch.randn(4, 32, 32, 32)
...
def test_rfnet_adapter_evaluate_batch():
    dummy = DummyRFNet(num_cls=4)
    adapter = RFNetAdapter(device="cpu", patch_size=(32, 32, 32), network=dummy)
    batch = {
...
def test_rfnet_adapter_checkpoint_loading():
    dummy1 = DummyRFNet(num_cls=4)
    adapter1 = RFNetAdapter(device="cpu", patch_size=(16, 16, 16), network=dummy1)
    ...
    try:
        ...
        dummy2 = DummyRFNet(num_cls=4)
        adapter2 = RFNetAdapter(weights_path=ckpt_path, device="cpu", patch_size=(16, 16, 16), network=dummy2)
...
def test_rfnet_adapter_postprocess_et():
    dummy = DummyRFNet(num_cls=4)
    adapter = RFNetAdapter(device="cpu", patch_size=(16, 16, 16), network=dummy)
    ...
    # Force ET suppression with large threshold
    pred_post = adapter.predict(x, postprocess_et=True, et_threshold=50000)
    assert np.sum(pred_post == 4) == 0


def test_rfnet_adapter_real_model_inference():
    """Verify inference with real official RFNet Model on a tiny 16x16x16 patch."""
    adapter = RFNetAdapter(device="cpu", patch_size=(16, 16, 16))
    x = torch.randn(4, 16, 16, 16)
    pred, logits = adapter.predict(x, mask="S1", return_logits=True)
    assert pred.shape == (16, 16, 16)
    assert logits.shape == (4, 16, 16, 16)
    unique_labels = set(np.unique(pred))
    assert unique_labels.issubset({0, 1, 2, 4})
